...
Function Get-TopProcess {
    ...
    Write-Verbose "Running Get-TopProcess"
    Write-Verbose "Getting $count top processes"
    $sb = {
        Param($count) Get-Process |
        Sort-Object -Property WS -Descending |
        Select-Object -First $count
    }
    #this is using a technique called splatting
    $params = @{
        Scriptblock  = $sb
        Computername = $Computername
        ArgumentList = $count
    }
    #add credential to the hashtable is specified
    #there are other ways to do this
    if ($credential) {
        Write-Verbose "Using alternate credential $($credential.username)"
        $params.add("Credential", $Credential)
    }

    Invoke-Command @params
    Write-Verbose "Ending Get-TopProcess"
...
Function Get-ComputerUptime {
    ...
    Write-Verbose "Running Get-ComputerUptime"
    ...
    Invoke-Command @params | Select-Object @{Name = "Computername"; Expression = { $_.CSName } },
    LastBootUpTime, @{Name = "Uptime"; Expression = { (Get-Date) - $_.LastBootUpTime } }

    Write-Verbose "Ending Get-ComputerUptime"
}
